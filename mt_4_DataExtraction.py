...
import pickle
with open(f"{op_dataFrame_dir}df_full.pickle", 'wb') as handle:
    pickle.dump(df_op, handle, protocol=pickle.HIGHEST_PROTOCOL)

# Saving dataframes
if 'gMAS' in dfo.columns:
    max_stress.to_csv(f"{op_dataFrame_dir}/df_maxLoad.csv")
    df_max_force_norm = min_max_scaling(max_stress)
    df_max_force_norm.to_csv(f"{op_dataFrame_dir}/df_maxLoad_norm.csv")

df_max_val.to_csv(f"{op_dataFrame_dir}/df_maxVal.csv")
df_max_val_idx.to_csv(f"{op_dataFrame_dir}/df_maxVal_idx.csv")
df_max_val_norm = min_max_scaling(df_max_val)
df_max_val_norm = df_max_val_norm.to_csv(f"{op_dataFrame_dir}/df_maxVal_norm.csv")